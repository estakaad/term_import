...
def create_meaning_tags(session, meaning_request_bodies_path, base_url, dataset_code):
    ...
    logger.info(f'Started processing meaning tags from file: {meaning_request_bodies_path}')
...
def create_lexeme_tag(session, request_body, base_url, dataset_code):
    parameters = {'crudRoleDataset': dataset_code}
    endpoint = base_url + "/api/lexeme_tag/create"
    ...
    if response.status_code >= 200 and response.status_code < 300:
        try:
            response_data = response.json()
            logger.info(f'Created lexeme tag {request_body}')
            return response_data['success']
        except json.JSONDecodeError:
            logger.warning(request_body)
            logger.warning(f"Failed to parse JSON from response when creating lexeme tag {request_body} "
                           f"Response text: {response.text}")

    else:
        logger.warning(request_body)
        logger.warning(f"Received non-200 response when creating lexeme tag. "
                       f"Status code: {response.status_code}, "
                       f"Response text: {response.text}")
    return None

def create_lexeme_tags(session, lexeme_request_bodies_path, base_url, dataset_code):

    logger.info(f'Started processing lexeme tags from file: {lexeme_request_bodies_path}')

    count_created_tags = 0

    with open(lexeme_request_bodies_path, 'r', encoding='utf-8') as f:
        lexeme_tags = json.load(f)

        for lexeme_tag in lexeme_tags:
            tag_status = create_lexeme_tag(session, lexeme_tag, base_url, dataset_code)
            if tag_status is True:
                count_created_tags += 1

    logger.info('Number of created lexeme tags: ' + str(count_created_tags))